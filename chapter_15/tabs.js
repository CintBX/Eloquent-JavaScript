/*
  ...
  Write a function, asTabs, that takes a DOM node and creates a tabbed interface showing
  ...
  the data-tabname attribute of the child. All but one of the original children should
  be hidden (given a display style of none). The currently visible node can be selected
...
function asTabs(node) {
  ...
  childElements.map(child => {
    if(child.nodeType == 1) elementNodes.push(child);
  });

  let button;
  for(let i = 0; i < elementNodes.length; i++) {
    button = document.createElement("button");
    button.innerText = elementNodes[i].getAttribute("data-tabname");
