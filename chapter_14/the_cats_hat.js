/*
  ...
  To make positioning multiple objects easier, it is probably a good idea to switch to
  absolute positioning.
  This means that top and left are counted relative to the top left of the document.
  
  To avoid using negative coordinates, which would cause the image to move outside of the
  visible page, you can add a fixed number of pixels to the position values.
*/
...
function animate(time) {
  ...
  cat.style.top = (Math.sin(angle) * 45 + 175) + "px";
  cat.style.left = (Math.cos(angle) * 140 + 395) + "px";
  hat.style.top = (Math.sin(angle) * 200 + 200) + "px";
  hat.style.left = (Math.cos(angle) * 375 + 415) + "px";
